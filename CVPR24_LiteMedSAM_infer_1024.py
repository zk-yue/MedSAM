...
def medsam_inference(medsam_model, img_embed, box_256, new_size, original_size):
    ...
    """
    box_torch = torch.as_tensor(box_256[None, None, ...], dtype=torch.float, device=img_embed.device) # (1, 4) -> torch.Size([1, 1, 1, 4])
    ...
    low_res_logits, iou = medsam_model.mask_decoder( # torch.Size([1, 1, 256, 256])
...
def MedSAM_infer_npz_2D(img_npz_file):
    npz_name = basename(img_npz_file) # '2DBox_Endoscopy_demo.npz'
    npz_data = np.load(img_npz_file, 'r', allow_pickle=True) # (H, W, 3)
    img_3c = npz_data['imgs'] # (H, W, 3) (1080, 1920, 3)
    assert np.max(img_3c)<256, f'input data should be in range [0, 255], but got {np.unique(img_3c)}'
    H, W = img_3c.shape[:2] # 1080, 1920
    boxes = npz_data['boxes'] # [564, 207, 1879, 1080]
    segs = np.zeros(img_3c.shape[:2], dtype=np.uint8) # (1080, 1920)

    ## preprocessing
    # img_256 = resize_longest_side(img_3c, 256)
    img_1024 = resize_longest_side(img_3c, 1024) # (1080, 1920, 3)-> (576, 1024, 3)
    newh, neww = img_1024.shape[:2] # 576, 1024
    img_1024_norm = (img_1024 - img_1024.min()) / np.clip(
        img_1024.max() - img_1024.min(), a_min=1e-8, a_max=None
    ) # normalize to [0, 1], (H, W, 3)
    # img_256_padded = pad_image(img_256_norm, 256)
    img_1024_padded = pad_image(img_1024_norm, 1024) # (576, 1024, 3)->(1024, 1024, 3)
    img_1024_tensor = torch.tensor(img_1024_padded).float().permute(2, 0, 1).unsqueeze(0).to(device) # torch.Size([1, 3, 1024, 1024])
    with torch.no_grad():
        image_embedding = repvit_medsam_model.image_encoder(img_1024_tensor) # torch.Size([1, 256, 64, 64])

    for idx, box in enumerate(boxes, start=1):
        box256 = resize_box_to_1024(box, original_size=(H, W)) # [564, 207, 1879, 1080]->[300, 110, 1002, 576]
    ...
    if fig_show:
        ...
        ax[1].set_title("img_1024")
        ax[2].imshow(img_1024_padded)
        ax[2].set_title("img_1024_padded")
        for i, box in enumerate(boxes):
            color = np.random.rand(3)
            box256 = resize_box_to_1024(box, original_size=(H, W)) # [564, 207, 1879, 1080]->[300, 110, 1002, 576]
            show_box(box256, ax[1], edgecolor=color)
            show_box(box256, ax[2], edgecolor=color)
            box_viz = box
            show_box(box_viz, ax[0], edgecolor=color)
            show_mask((segs == i+1).astype(np.uint8), ax[0], mask_color=color)
        plt.show()
...
def MedSAM_infer_npz_3D(img_npz_file):
    ...
    for idx, box3D in enumerate(boxes_3D, start=1):
        ...
        for z in range(z_middle, z_max):
            ...
            img_1024 = pad_image(img_1024,1024)
            
            # convert the shape to (3, H, W)
            img_1024_tensor = torch.tensor(img_1024).float().permute(2, 0, 1).unsqueeze(0).to(device)
            # get the image embedding
            with torch.no_grad():
                image_embedding = repvit_medsam_model.image_encoder(img_1024_tensor) # (1, 256, 64, 64)
            if z == z_middle:
                box_256 = resize_box_to_1024(mid_slice_bbox_2d, original_size=(H, W))
            else:
                pre_seg = segs_3d_temp[z-1, :, :]
                pre_seg256 = resize_longest_side(pre_seg,1024)
...
if __name__ == '__main__':
    ...
    for img_npz_file in tqdm(img_npz_files):
        start_time = time()
        if basename(img_npz_file).startswith('3D'): # basename返回路径名的最后一部分  startswith('3D') 会检查基本文件名是否以 '3D' 开始
