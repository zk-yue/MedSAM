...
class NpyDataset(Dataset):
    def __init__(self, data_root, image_size=256, teacher_image_size=1024, data_aug=False):
        ...
        self.teacher_target_length =teacher_image_size
    ...
    def __getitem__(self, index):
        ...
        teacher_img_resize = self.teacher_resize_longest_side(img_3c)
        # Resizing
        teacher_img_resize = (teacher_img_resize - teacher_img_resize.min()) / np.clip(teacher_img_resize.max() - teacher_img_resize.min(), a_min=1e-8,
        ...
        teacher_img_padded = self.teacher_pad_image(teacher_img_resize)
        # convert the shape to (3, H, W)
        img_padded = np.transpose(img_padded, (2, 0, 1))  # (3, 256, 256)
        teacher_img_padded = np.transpose(teacher_img_padded, (2, 0, 1))  # (3, 256, 256)
        assert np.max(img_padded) <= 1.0 and np.min(img_padded) >= 0.0, 'image should be normalized to [0, 1]'
        assert np.max(teacher_img_padded) <= 1.0 and np.min(teacher_img_padded) >= 0.0, 'image should be normalized to [0, 1]'

        return {
            "image": torch.tensor(img_padded).float(),
            "teacher_image":torch.tensor(teacher_img_padded).float(),
    ...
    def teacher_resize_longest_side(self, image):  # 其最长的边达到指定的长度 self.target_length。

        """
        Expects a numpy array with shape HxWxC in uint8 format.
        """
        long_side_length = self.teacher_target_length
        oldh, oldw = image.shape[0], image.shape[1]
        scale = long_side_length * 1.0 / max(oldh, oldw)
        newh, neww = oldh * scale, oldw * scale
        neww, newh = int(neww + 0.5), int(newh + 0.5)
    ...
    def teacher_pad_image(self, image):  # 填充图片到指定H,W
        """
        Expects a numpy array with shape HxWxC in uint8 format.
        """
        # Pad
        h, w = image.shape[0], image.shape[1]
        padh = self.teacher_image_size - h
        padw = self.teacher_image_size - w
        if len(image.shape) == 3:  ## Pad image
            image_padded = np.pad(image, ((0, padh), (0, padw), (0, 0)))
        else:  ## Pad gt mask
            image_padded = np.pad(image, ((0, padh), (0, padw)))
...
for epoch in range(start_epoch , num_epochs):
    ...
    for step, batch in enumerate(pbar):
        ...
        teacher_image = teacher_image.to(device)
        with torch.no_grad():
            # print(f"Input shape: {image.shape}")
            teacher_output = medsam_model.image_encoder(teacher_image)
